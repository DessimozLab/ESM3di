#!/bin/bash
# Train ESM++ small models on all 10 order-level datasets
# Usage: ./train_all_orders.sh [--resume]
...
echo "=============================================="
echo "Training ESM++ small on 10 viral orders"
echo "=============================================="
echo ""
...
for order in "${ORDERS[@]}"; do
    echo "=============================================="
    echo "Training: $order"
    echo "Config: config_esmpp_small_${order}.json"
    echo "=============================================="
    
    RESUME_ARG=""
    if [[ "$RESUME_MODE" == true ]]; then
        ckpt_dir="checkpoints_esmpp_small_${order}"
    ...
    python -m esm3di.esmretrain \
        --config "config_esmpp_small_${order}.json" \
        $RESUME_ARG
...
for order in "${ORDERS[@]}"; do
    echo "  - checkpoints_esmpp_small_${order}/"
done
