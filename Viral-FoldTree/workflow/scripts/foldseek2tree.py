...
# old replaced by the function below
def Tajima_dist( kn_ratio,bfactor=19/20, iter = 100 ):
	"""
	Jukes-Cantor evolutionary distance model using its Taylor series expansion.
	"""
...
def jukes_cantor_taylor(kn_ratio, bfactor=19/20, iter=100):
    """
    Calculates Jukes-Cantor evolutionary distance using Taylor series expansion.
    """
    # Create an array of exponents [1, 2, ..., iter-1]
    i = np.arange(1, iter)
    
    # Use broadcasting to calculate the series for the entire matrix at once
    # Adds a new axis to kn_ratio to align with the 'i' vector
    series = (kn_ratio[..., np.newaxis] ** i) / (bfactor ** (i - 1) * i)
    
    # Sum across the last axis (the terms of the Taylor series)
    jk_dist = np.sum(series, axis=-1)
    
    # Ensure the diagonal remains 0
    np.fill_diagonal(jk_dist, 0)
...
def runFoldseek_allvall( structfolder , outfolder , foldseekpath = '../foldseek/bin/foldseek' , maxseqs = 3000):
	...
	foldseekpath + " easy-search " + structfolder + " "+ structfolder +" "+ outfolder+"/allvall.csv " +  structfolder+"/tmp --format-output 'query,target,fident,alnlen,mismatch,gapopen,qstart,qend,tstart,tend,evalue,bits' --exhaustive-search --alignment-type 2" 
...
def runFoldseek_allvall_EZsearch(infolder , outpath , foldseekpath = '../foldseek/bin/foldseek'):
	...
	args = foldseekpath + ' easy-search ' + infolder + ' ' + infolder +' '+ outpath + " tmp --format-output 'query,target,fident,alnlen,mismatch,gapopen,qstart,qend,tstart,tend,evalue,bits' --exhaustive-search "
	p = runargs(args)
	return outpath
