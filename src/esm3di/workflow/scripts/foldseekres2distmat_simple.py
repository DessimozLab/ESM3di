...
for i,k in enumerate(matrices):
    matrices[k] /= 2 # ensure matrices are symmetric
    matrices[k] = 1-matrices[k] # from similarity score to distance
    print(matrices[k], np.amax(matrices[k]), np.amin(matrices[k]) )
    if k == 'fident':
        #bfactor=19/20
        bfactor=.93 # TODO: WHY not 19/20?
    else:
        bfactor=1
    tajima =  foldseek2tree.jukes_cantor_taylor(matrices[k] + 10 **-5 , bfactor=bfactor )    
