...
This module provides an iterative refinement head (PonderNet-style)
...
class IterativeTransformerClassificationHead(nn.Module):
    ...
    def forward(
        ...
        if return_halt_info:
            ...
            with torch.no_grad():
                valid_mask = attention_mask if attention_mask is not None else torch.ones(batch_size, seq_len,
                                                                                          device=device)
